...
| `data-tilt="N"` (on the filmstrip plate media-frames) | 3D pointer tilt: the element rotates up to ±N° toward the cursor in perspective (`perspective` on the plate, `quickTo` on rotationX/Y). Composes with the plate's velocity-skew and the image zoom as separate transform owners. Pointer-driven, so it does not jitter during the pin. Fine pointers only. |
...
| `data-chapter` (on an `aria-hidden` interstitial band before a movement) | between-movements chapter divider with a choreographed entrance on scroll-in: the giant ghost numeral (`.chapter__ghost`) scales + slides + fades in, the kicker (`.chapter__kicker`) rises, and the title (`.chapter__title`) mask-reveals line by line. SplitText + the whole entrance are built in `buildLineMotion` (so line breaks use final font metrics) via one timeline per band. Not built under reduced-motion, so the parts stay at their visible CSS defaults. Placed before movements 04 and 06. |
